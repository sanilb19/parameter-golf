...
OUT_DIR = ROOT / "logs" / "overnight_local_ab"
...
CANDIDATES = [
    Candidate("rope_20000", "ROPE_BASE=20000", {"ROPE_BASE": "20000.0"}),
    Candidate("rope_5000", "ROPE_BASE=5000", {"ROPE_BASE": "5000.0"}),
    Candidate("muon_3", "MUON_BACKEND_STEPS=3", {"MUON_BACKEND_STEPS": "3"}),
    Candidate("muon_7", "MUON_BACKEND_STEPS=7", {"MUON_BACKEND_STEPS": "7"}),
    Candidate("matrix_lr_035", "MATRIX_LR=0.035", {"MATRIX_LR": "0.035"}),
    Candidate("matrix_lr_045", "MATRIX_LR=0.045", {"MATRIX_LR": "0.045"}),
    Candidate("warmdown_800", "WARMDOWN_ITERS=800", {"WARMDOWN_ITERS": "800"}),
    Candidate("warmdown_2000", "WARMDOWN_ITERS=2000", {"WARMDOWN_ITERS": "2000"}),
]
...
def main() -> None:
    ...
    for idx, candidate in enumerate(CANDIDATES, start=1):
        ...
        if should_stop():
            ...
            break
        base_a_id = f"ab_{idx:02d}_{candidate.key}_base_a"
        cand_id = f"ab_{idx:02d}_{candidate.key}_cand"
        base_b_id = f"ab_{idx:02d}_{candidate.key}_base_b"
