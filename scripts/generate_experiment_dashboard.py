...
def html_table(rows: list[Experiment]) -> str:
    ...
    for e in rows:
        ...
        out.append(
            "<tr>"
            f"<td>{html.escape(e.batch)}</td>"
            f"<td>{html.escape(e.phase)}</td>"
            f"<td>{html.escape(e.name)}</td>"
            f"<td>{html.escape(e.status)}</td>"
            f"<td>{'' if e.val_bpb is None else f'{e.val_bpb:.8f}'}</td>"
            f"<td>{'' if e.artifact is None else f'{e.artifact:,}'}</td>"
            f"<td>{'' if e.tok_s is None else f'{e.tok_s:.0f}'}</td>"
            f"<td>{'' if e.delta_bpb is None else f'{e.delta_bpb:+.8f}'}</td>"
...
def main() -> None:
    ...
    html_parts = [
        ...
        "th,td{padding:8px 10px;border-bottom:1px solid #eef2f7;font-size:13px;text-align:left;vertical-align:top}",
        ...
        "<div class='grid'>",
        f"<div class='card'><div class='muted'>Experiments</div><div style='font-size:32px;font-weight:700'>{len(experiments)}</div></div>",
        f"<div class='card'><div class='muted'>Batches</div><div style='font-size:32px;font-weight:700'>{len(batch_counts)}</div></div>",
        f"<div class='card'><div class='muted'>Best val_bpb</div><div style='font-size:32px;font-weight:700'>{valid_metric_runs[0].val_bpb:.6f}</div><div class='small'>{html.escape(valid_metric_runs[0].name)}</div></div>",
        f"<div class='card'><div class='muted'>Parameters touched</div><div style='font-size:32px;font-weight:700'>{len(param_values)}</div></div>",
        "</div>",
        "<div class='grid'>",
    ]
    for status, count in sorted(status_counts.items()):
        html_parts.append(f"<div class='card'><div class='muted'>{html.escape(status)}</div><div style='font-size:26px;font-weight:700'>{count}</div></div>")
