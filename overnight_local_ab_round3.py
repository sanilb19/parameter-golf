...
OUT_DIR = ROOT / "logs" / "overnight_local_ab_round3"
...
CANDIDATES = [
    Candidate("warmdown_2400_recheck", "WARMDOWN_ITERS=2400", {"WARMDOWN_ITERS": "2400"}),
    Candidate("warmdown_2800", "WARMDOWN_ITERS=2800", {"WARMDOWN_ITERS": "2800"}),
    Candidate("warmdown_3600", "WARMDOWN_ITERS=3600", {"WARMDOWN_ITERS": "3600"}),
    Candidate("warmdown_4000", "WARMDOWN_ITERS=4000", {"WARMDOWN_ITERS": "4000"}),
...
def write_summary(rows: list[dict[str, object]]) -> None:
    lines = [
        "# Overnight Local A/B Round 3 Summary",
        "",
        "- Focus: warmdown refinement around the provisional 3200-step base",
...
def main() -> None:
    log("overnight_local_ab_round3_start")
    ...
    for idx, candidate in enumerate(CANDIDATES, start=1):
        ...
        if should_stop():
            ...
            break
        base_a_id = f"r3_{idx:02d}_{candidate.key}_base_a"
        cand_id = f"r3_{idx:02d}_{candidate.key}_cand"
        base_b_id = f"r3_{idx:02d}_{candidate.key}_base_b"
    ...
    log("overnight_local_ab_round3_done")
