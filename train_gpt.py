...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, mlp_mult: int, act: str):
        super().__init__()
        hidden = mlp_mult * dim
        if act not in {"relu2", "silu", "silu2"}:
            raise ValueError(f"Unsupported MLP_ACT={act!r}; expected one of relu2, silu, silu2")
        self.act = act
...
def main() -> None:
    ...
    log0(
        f"train_microbatch_tokens_per_gpu:{args.train_batch_tokens // (world_size * grad_accum_steps)}"
    )
