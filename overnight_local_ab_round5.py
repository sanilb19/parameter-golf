...
OUT_DIR = ROOT / "logs" / "overnight_local_ab_round5"
...
CANDIDATES = [
    Candidate("tied_embed_lr_004_recheck", "TIED_EMBED_LR=0.04", {"TIED_EMBED_LR": "0.04"}),
    Candidate("tied_embed_lr_0045", "TIED_EMBED_LR=0.045", {"TIED_EMBED_LR": "0.045"}),
    Candidate("tied_embed_lr_0035", "TIED_EMBED_LR=0.035", {"TIED_EMBED_LR": "0.035"}),
    Candidate("num_kv_heads_2", "NUM_KV_HEADS=2", {"NUM_KV_HEADS": "2"}),
    Candidate("num_kv_heads_8", "NUM_KV_HEADS=8", {"NUM_KV_HEADS": "8"}),
]
...
def write_summary(rows: list[dict[str, object]]) -> None:
    lines = [
        "# Overnight Local A/B Round 5 Summary",
        "",
        "- Focus: tied-embedding-LR recheck plus a first KV-head architecture probe",
...
def notify_completion(rows: list[dict[str, object]], status_text: str) -> None:
    if rows:
        ...
        body = f"{status_text}\nround5 completed with no candidate rows\nsummary={SUMMARY_MD}"
    req = urllib.request.Request(
        f"https://ntfy.sh/{NTFY_TOPIC}",
        data=body.encode("utf-8"),
        headers={
            "Title": "Parameter Golf Round 5 Complete",
            "Priority": "default",
            "Tags": "robot,abacus",
        },
        method="POST",
...
def main() -> None:
    log("overnight_local_ab_round5_start")
    ...
    for idx, candidate in enumerate(CANDIDATES, start=1):
        ...
        if should_stop():
            ...
            break
        base_a_id = f"r5_{idx:02d}_{candidate.key}_base_a"
        cand_id = f"r5_{idx:02d}_{candidate.key}_cand"
        base_b_id = f"r5_{idx:02d}_{candidate.key}_base_b"
    ...
    log("overnight_local_ab_round5_done")
    write_summary(rows)
    notify_completion(rows, "round5 complete")
...
if __name__ == "__main__":
    try:
        ...
        log(f"round5_fatal error={exc}")
        notify_completion(load_existing_rows(), f"round5 failed: {exc}")
        raise
