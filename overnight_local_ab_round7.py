...
OUT_DIR = ROOT / "logs" / "overnight_local_ab_round7"
...
STAGE1_CANDIDATES = [
    Candidate("embed_0035", "TIED_EMBED_LR=0.035", {"TIED_EMBED_LR": "0.035"}, "stage1"),
    Candidate("embed_0045", "TIED_EMBED_LR=0.045", {"TIED_EMBED_LR": "0.045"}, "stage1"),
    Candidate("warmdown_2800", "WARMDOWN_ITERS=2800", {"WARMDOWN_ITERS": "2800"}, "stage1"),
    Candidate("warmdown_4000", "WARMDOWN_ITERS=4000", {"WARMDOWN_ITERS": "4000"}, "stage1"),
    Candidate("kv_8", "NUM_KV_HEADS=8", {"NUM_KV_HEADS": "8"}, "stage1"),
    Candidate("kv_1", "NUM_KV_HEADS=1", {"NUM_KV_HEADS": "1"}, "stage1"),
    Candidate("mlp_mult_1", "MLP_MULT=1", {"MLP_MULT": "1"}, "stage1"),
    Candidate("mlp_mult_3", "MLP_MULT=3", {"MLP_MULT": "3"}, "stage1"),
    Candidate("layers_8", "NUM_LAYERS=8", {"NUM_LAYERS": "8"}, "stage1"),
    Candidate("layers_10", "NUM_LAYERS=10", {"NUM_LAYERS": "10"}, "stage1"),
    Candidate("dim_384", "MODEL_DIM=384 NUM_HEADS=8 NUM_KV_HEADS=4", {"MODEL_DIM": "384", "NUM_HEADS": "8", "NUM_KV_HEADS": "4"}, "stage1"),
    Candidate("dim_640", "MODEL_DIM=640 NUM_HEADS=10 NUM_KV_HEADS=5", {"MODEL_DIM": "640", "NUM_HEADS": "10", "NUM_KV_HEADS": "5"}, "stage1"),
    Candidate("embed_004_warmdown_2800", "TIED_EMBED_LR=0.04 + WARMDOWN_ITERS=2800", {"TIED_EMBED_LR": "0.04", "WARMDOWN_ITERS": "2800"}, "stage1"),
]
...
def write_summary(rows: list[dict[str, object]]) -> None:
    lines = [
        "# Overnight Local A/B Round 7 Summary",
        "",
        "- Focus: fast 6-minute silu-centered ablations plus seed reruns and longer confirmations",
...
def notify_completion(rows: list[dict[str, object]], status_text: str) -> None:
    if rows:
        ...
        body = f"{status_text}\nround7 completed with no candidate rows\nsummary={SUMMARY_MD}"
    req = urllib.request.Request(
        f"https://ntfy.sh/{NTFY_TOPIC}",
        data=body.encode("utf-8"),
        headers={
            "Title": "Parameter Golf Round 7 Complete",
            "Priority": "default",
            "Tags": "robot,abacus",
        },
        method="POST",
...
def run_candidate_block(candidate: Candidate, idx: int, seed_override: str | None = None, long_confirm: bool = False) -> dict[str, object]:
    ...
    if long_confirm:
        extra["MAX_WALLCLOCK_SECONDS"] = "600"
        phase = f"{phase}_10m"
    ...
    if long_confirm:
        suffix += "_10m"
    base_a_id = f"r7_{idx:02d}_{candidate.key}{suffix}_base_a"
    cand_id = f"r7_{idx:02d}_{candidate.key}{suffix}_cand"
    base_b_id = f"r7_{idx:02d}_{candidate.key}{suffix}_base_b"

    base_a = run_one(base_a_id, {"SEED": seed_value, "MAX_WALLCLOCK_SECONDS": extra.get("MAX_WALLCLOCK_SECONDS", BASE_ENV["MAX_WALLCLOCK_SECONDS"])})
    time.sleep(COOLDOWN_SECONDS)
    candidate_metrics = run_one(cand_id, extra)
    time.sleep(COOLDOWN_SECONDS)
    base_b = run_one(base_b_id, {"SEED": seed_value, "MAX_WALLCLOCK_SECONDS": extra.get("MAX_WALLCLOCK_SECONDS", BASE_ENV["MAX_WALLCLOCK_SECONDS"])})
...
def stage_rank(rows: list[dict[str, object]], phase: str) -> list[dict[str, object]]:
    subset = [row for row in rows if row["phase"] == phase and row["status"] != "contaminated"]
    return sorted(subset, key=lambda row: (float(row["delta_bpb"]), float(row["candidate_bpb"])))


def main() -> None:
    log("overnight_local_ab_round7_start")
    ...
    stage2_candidates = []
    for row in ranked_stage1[:MAX_STAGE2_BLOCKS]:
        for candidate in STAGE1_CANDIDATES:
            if candidate.description == row["candidate"]:
                stage2_candidates.append(Candidate(candidate.key, candidate.description, candidate.env, "stage2"))
                break
    ...
    ranked_stage2 = stage_rank(rows, f"stage2_seed{SECOND_SEED}")
    if not ranked_stage2:
        ranked_stage2 = ranked_stage1
    stage3_candidates = []
    for row in ranked_stage2[:MAX_STAGE3_BLOCKS]:
        for candidate in STAGE1_CANDIDATES:
            if candidate.description == row["candidate"]:
                stage3_candidates.append(Candidate(candidate.key, candidate.description, candidate.env, "stage3"))
                break
    ...
    for offset, candidate in enumerate(stage3_candidates, start=1):
        ...
        if should_stop():
            ...
            break
        row = run_candidate_block(candidate, len(STAGE1_CANDIDATES) + MAX_STAGE2_BLOCKS + offset, seed_override=THIRD_SEED, long_confirm=True)
        rows.append(row)
        append_result(row)
        write_summary(rows)
        log(
    ...
    log("overnight_local_ab_round7_done")
    write_summary(rows)
    notify_completion(rows, "round7 complete")
...
if __name__ == "__main__":
    try:
        ...
        log(f"round7_fatal error={exc}")
        notify_completion(load_existing_rows(), f"round7 failed: {exc}")
        raise
