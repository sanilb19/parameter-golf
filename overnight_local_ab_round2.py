...
OUT_DIR = ROOT / "logs" / "overnight_local_ab_round2"
...
CANDIDATES = [
    Candidate("warmdown_2000_confirm", "WARMDOWN_ITERS=2000 (confirm)", {"WARMDOWN_ITERS": "2000"}),
    Candidate("warmdown_1600", "WARMDOWN_ITERS=1600", {"WARMDOWN_ITERS": "1600"}),
    Candidate("warmdown_2400", "WARMDOWN_ITERS=2400", {"WARMDOWN_ITERS": "2400"}),
    Candidate("warmdown_3200", "WARMDOWN_ITERS=3200", {"WARMDOWN_ITERS": "3200"}),
    Candidate(
        "warmdown_2000_muon_7",
        "WARMDOWN_ITERS=2000 + MUON_BACKEND_STEPS=7",
        {"WARMDOWN_ITERS": "2000", "MUON_BACKEND_STEPS": "7"},
    ),
    Candidate(
        "warmdown_2000_rope_5000",
        "WARMDOWN_ITERS=2000 + ROPE_BASE=5000",
        {"WARMDOWN_ITERS": "2000", "ROPE_BASE": "5000.0"},
    ),
]
...
def write_summary(rows: list[dict[str, object]]) -> None:
    lines = [
        "# Overnight Local A/B Round 2 Summary",
        "",
        "- Focus: warmdown confirmation and neighborhood sweep",
...
def main() -> None:
    log("overnight_local_ab_round2_start")
    ...
    for idx, candidate in enumerate(CANDIDATES, start=1):
        ...
        if should_stop():
            ...
            break
        base_a_id = f"r2_{idx:02d}_{candidate.key}_base_a"
        cand_id = f"r2_{idx:02d}_{candidate.key}_cand"
        base_b_id = f"r2_{idx:02d}_{candidate.key}_base_b"
    ...
    log("overnight_local_ab_round2_done")
